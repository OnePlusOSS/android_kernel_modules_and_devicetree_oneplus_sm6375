...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 0))
	load = clamp(load, uclamp_eff_value(tsk, UCLAMP_MIN), uclamp_eff_value(tsk, UCLAMP_MAX));
#endif
...
static struct load_weight sa_new_weight(struct sched_entity *se)
{
	...
	if ((sysctl_sched_assist_enabled < 2) || !entity_is_task(se)
		...
		list_for_each_safe(pos, n, &rq->ux_thread_list) {
			if (pos == &p->ux_entry) {
				...
				if (get_ux_state_type(p) == UX_STATE_INHERIT &&
				    now - p->inherit_ux_start >
				    ux_max_dynamic_granularity) {
...
void drop_ux_task_cpus(struct task_struct *p, struct cpumask *lowest_mask)
{
	...
	while (cpu < nr_cpu_ids) {
		...
		if (sched_assist_scene(SA_SLIDE) || sched_assist_scene(SA_INPUT)
			...
			sf = is_sf(p);
			if (sf
			    && is_task_util_over(p, sysctl_boost_task_threshold)
			    && oplus_is_min_capacity_cpu(cpu))
				cpumask_clear_cpu(cpu, lowest_mask);
		}
#endif
...
void sched_assist_target_comm(struct task_struct *task)
{
	...
	if (task->pid == task->tgid
	    && (task_uid(task).val == 1000)
	    && !strncmp(task->comm, SURFACE_FLINGER_NAME, TASK_COMM_LEN)) {
	...
	if ((task_uid(task).val == 1000)
	    && !strncmp(task->comm, SF_RENDER_ENGINE_NAME, TASK_COMM_LEN)) {
		if (sSF_union[1] == -1) {
...
#ifdef CONFIG_OPLUS_FEATURE_AUDIO_OPT
	if (!sysctl_sched_impt_tgid
	    && test_task_identify_ux(se_task, SA_TYPE_ID_CAMERA_PROVIDER)) {
#else
			      ...
			      struct task_struct *curr_task)
{
	...
	if (wake_ux && !(wake_task->ux_state & SA_TYPE_HEAVY)
	    && (curr_task->ux_state & SA_TYPE_HEAVY))
		return true;
...
bool is_task_util_over(struct task_struct *task, int threshold)
{
	...
	if (unlikely(task_index_of_sf_union(task) >= 0)) {
		unsigned long util =
		    sSF_union_util[0] + sSF_union_util[1] + sSF_union_util[2];
...
static inline int get_task_cls_for_scene(struct task_struct *task)
{
	...
	if ((sched_assist_scene(SA_LAUNCHER_SI) || sched_assist_scene(SA_SLIDE)
	     || sched_assist_scene(SA_INPUT) || sched_assist_scene(SA_ANIM))
...
static inline bool is_ux_task_prefer_cpu_for_scene(struct task_struct *task,
						   unsigned int cpu)
{
	struct ux_sched_cputopo ux_cputopo = ux_sched_cputopo;
...
bool sched_assist_task_misfit(struct task_struct *task, int cpu, int flag)
{
	...
	if (test_task_identify_ux(task, SA_TYPE_ID_CAMERA_PROVIDER)
	    && is_task_util_over(task, THRESHOLD_BOOST)
	    && oplus_is_min_capacity_cpu(cpu)) {
		return true;
	}
		      ...
		      struct task_struct *ux_task, int reset_type)
{
	...
	inherit_task->ux_state =
	    (inherit_task->ux_state & ~SCHED_ASSIST_UX_MASK) | reset_inherit;
...
u64 sf_union_ux_load(struct task_struct *tsk, u64 timeline)
{
	...
	if (unlikely(index >= 0)) {
		...
		return sSF_union_ux_load[0] + sSF_union_ux_load[1] +
			sSF_union_ux_load[2];
	}
	return timeline;
}
				  ...
				  size_t count, loff_t *ppos)
{
	...
	len =
	    snprintf(buffer, sizeof(buffer),
		     "pid=%d ux_state=%d inherit=%lld(fu:%d mu:%d rw:%d bi:%d) cpuset_id=%d\n",
		     ...
		     test_inherit_ux(task, INHERIT_UX_BINDER), cpuset_id);
					 ...
					 size_t count, loff_t *ppos)
{
	...
	len =
	    snprintf(buffer, sizeof(buffer), "top(%d %u) au(%d %d)\n",
		     save_top_app_tgid, top_app_type, save_impt_tgid[0],
		     save_impt_tgid[1]);
...
static int __init oplus_sched_assist_init(void)
{
	...
	proc_node =
	    proc_create("sched_impt_task", 0666, d_sched_assist,
...
{
	if (((task->prio == ANDROID_PRIORITY_URGENT_AUDIO)
...
bool sched_assist_pick_next_task_opt(struct cfs_rq *cfs_rq,
				     struct sched_entity **se)
{
	if (cfs_rq->next && oplus_entity_is_task(cfs_rq->next)) {
		struct task_struct *tsk = task_of(cfs_rq->next);
...
void try_to_mark_task_type(struct task_struct *tsk, int type)
{
	...
	switch (type) {
	case TASK_SMALL:
		if ((tsk->oplus_task_info.sa_info[TST_EXEC][TASK_INFO_SAMPLE - 1] != 0)
		    && (tsk->oplus_task_info.sa_info[TST_SLEEP][TASK_INFO_SAMPLE - 1] != 0)) {
...
void update_sa_task_stats(struct task_struct *tsk, u64 delta_ns, int stats_type)
{
	...
	if (tsk->oplus_task_info.im_small
	    && (!is_sched_boost_group(tsk) || !sched_assist_scene(SA_CAMERA))) {
