...
bool _slide_task_misfit(struct task_struct *p, int cpu)
{
	...
	return (is_task_util_over(p, sysctl_boost_task_threshold)
		&& cpu < num_mincpu);
}
...
u64 _slide_get_boost_load(int cpu)
{
	...
	if ((sched_assist_scene(SA_SLIDE) || sched_assist_scene(SA_INPUT)
	     || sched_assist_scene(SA_ANIM)) && ux_task_load[cpu]) {
...
void adjust_sched_assist_input_ctrl(void)
{
	...
	if (sysctl_input_boost_enabled) {
		if (sysctl_slide_boost_enabled
		    ...
		    sched_assist_input_boost_duration) {
...
void slide_calc_boost_load(struct rq *rq, unsigned int *flag, int cpu)
{
	...
	if (keep_slide_nolimit(rq->curr) || sched_assist_scene(SA_SLIDE)
						 ...
						 size_t *lenp, loff_t *ppos)
{
	...
	sched_assist_input_boost_duration = jiffies_to_msecs(jiffies) +
		DEFAULT_INPUT_BOOST_DURATION / sched_assist_ib_duration_coedecay;
out:
...
int sysctl_sched_boost_task_threshold_handler(struct ctl_table *table,
					      ...
					      size_t *lenp, loff_t *ppos)
{
	...
	if (write)
		sched_assist_systrace(sysctl_boost_task_threshold,
				      "ux threshold");

	return ret;
}

int sysctl_sched_animation_type_handler(struct ctl_table *table, int write,
					void __user *buffer, size_t *lenp,
					loff_t *ppos)
{
	int ret;
